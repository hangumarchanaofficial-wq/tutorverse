-- Idempotency claim-first pattern: row is inserted before the RPC runs,
-- with order_id NULL until the order is created. This eliminates the
-- create-twice race that the prior 23505-after-insert fallback could not.
...
-- Update RPC to include apartment + postal code in the human-readable
-- shipping_address (full structured copy still lives in shipping_payload).
...
begin
  ...
  v_shipping_address :=
    ...
    case when coalesce(p_shipping->>'apartment', '') <> ''
      then ', ' || (p_shipping->>'apartment')
      else ''
    end || ', ' ||
    coalesce(p_shipping->>'city', '') || ', ' ||
    coalesce(p_shipping->>'province', '') || ' ' ||
    coalesce(p_shipping->>'postalCode', '') || ', ' ||
