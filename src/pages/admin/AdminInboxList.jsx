    ...
    <div className="space-y-5">
      ...
      <div className="flex min-h-[560px] flex-col overflow-hidden rounded-2xl border border-[#263145] bg-[#121b2e] shadow-[0_24px_48px_rgba(0,0,0,0.25)] lg:flex-row">
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-[#121b2e]">
          <div className="flex items-center justify-between gap-2 border-b border-[#263145] px-3 py-2.5 sm:gap-3 sm:px-4 sm:py-3">
            <label className="hidden cursor-pointer items-center gap-2 text-[11px] text-[#8b95a7] sm:flex">
              <input
                ref={selectAllRef}
                type="checkbox"
            ...
            <div className="flex flex-1 items-center justify-end gap-1 text-[11px] text-[#8b95a7] sm:flex-initial sm:justify-start">
          ...
          <div className="relative border-b border-[#263145] px-3 py-2 sm:px-4">
            <Search className="pointer-events-none absolute left-6 top-1/2 h-4 w-4 -translate-y-1/2 text-[#6b7280] sm:left-7" strokeWidth={1.75} />
            <input
              type="search"
              value={searchQuery}
              onChange={(e) => { setSearchQuery(e.target.value); setPage(1); }}
              placeholder="Search messages…"
              className="w-full rounded-lg border border-[#263145] bg-[#0f1726] py-2 pl-9 pr-3 text-sm text-[#f8fafc] placeholder-[#6b7280] outline-none focus:border-[#d8b84f]/50 sm:pl-10"
            />
          </div>
            ...
            {pageSlice.length === 0 ? (
              ...
              pageSlice.map((m) => (
                <div
                  ...
                  className={`flex cursor-pointer items-start gap-3 px-3 py-3 transition hover:bg-[#182238]/80 sm:px-4 sm:py-3.5 ${
                    m.unread ? "bg-[#0f1726]/40" : ""
                  }`}
                >
                  {/* Desktop: checkbox, star, snooze */}
                  <div className="hidden shrink-0 items-start gap-1 pt-0.5 sm:flex">
                    <input
                      type="checkbox"
                      className="mt-1 rounded border-[#334155] bg-[#182238]"
                    ...
                    <button
                      type="button"
                      className="text-[#8b95a7] hover:text-[#d8b84f]"
                    ...
                    <button
                      type="button"
                      className={`rounded p-0.5 hover:bg-[#263145] ${
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <span
                        className={`min-w-0 truncate text-sm ${
                          m.unread ? "font-semibold text-[#f8fafc]" : "font-medium text-[#e5e7eb]"
                        }`}
                      >
                        {m.from}
                      </span>
                      <div className="flex shrink-0 items-center gap-1.5 text-[#8b95a7] sm:hidden">
                        {m.hasAttachment && <Paperclip className="h-3.5 w-3.5" strokeWidth={2} />}
                        <span className="text-[10px] tabular-nums whitespace-nowrap">{fmtShortDate(m.date)}</span>
                      </div>
                    </div>
                    <p
                      className={`mt-0.5 truncate text-sm ${
                        m.unread ? "font-medium text-[#f8fafc]" : "text-[#c3cad9]"
                      }`}
                    >
                      {m.subject}
                    </p>
                    <p className="mt-0.5 line-clamp-1 text-xs text-[#8b95a7]">{m.snippet}</p>
                  ...
                  <button
                    type="button"
                    className="shrink-0 self-center text-[#8b95a7] hover:text-[#d8b84f] sm:hidden"
                    aria-label={m.starred ? "Unstar" : "Star"}
                    onClick={(e) => toggleStar(m.id, e)}
                  >
                    <Star className={`h-4 w-4 ${m.starred ? "fill-[#d8b84f] text-[#d8b84f]" : ""}`} strokeWidth={1.75} />
          ...
          <div className="flex items-center justify-between gap-2 border-t border-[#263145] px-3 py-2.5 sm:px-4 sm:py-3">
